...
- **Google Sheet mirror — removed** (user request): the app previously best-effort-mirrored `[email, date]` to a Google Sheet via `src/lib/google-sheets.ts` (Google service account JWT + Sheets API). Supabase is now the sole subscriber store; the Sheets integration, its `google-auth-library` dependency, and the `GOOGLE_SERVICE_ACCOUNT_EMAIL` / `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` / `GOOGLE_SHEETS_RANGE` env vars were removed. Existing subscriber rows already in Supabase were left untouched. The retired file was moved to `trash-can/google-sheets.ts` per this project's no-hard-deletes rule rather than deleted outright.
