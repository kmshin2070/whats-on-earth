...
10. ✅ Done — `src/data/issues.ts` now holds 9 real, current (2026) economic issues researched via WebSearch, each screened against PRD §5-1's include/exclude criteria and cross-verified against 2-4 independent approved outlets per issue (Bloomberg, The Washington Post, Nikkei Asia, South China Morning Post — Reuters/FT/WSJ/NYT/Guardian/Economist block search-crawler access, so sourcing concentrated on the four reachable approved outlets). All source URLs checked reachable (Bloomberg/Nikkei/SCMP via direct HTTP 200; Washington Post confirmed genuine via independent cross-search, since it blocks direct bot fetches with a 403 but is real and indexed).
